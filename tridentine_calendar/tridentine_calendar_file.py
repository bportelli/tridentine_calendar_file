...
class LiturgicalCalendarEvent:
    ...
    def is_fixed(self):
        ...
        """
        dateformat = '%B %#d' if (os.name == 'nt') else '%B %-d'  
        date_str = self.date.strftime(dateformat)
...
class LiturgicalYear:
    ...
    def to_list(self, html_formatting=False):
        """Output out the calendar to a list of dicts (one dict per event).
        ...
        for date in iterate_liturgical_year(self.year):
            for i, elem in enumerate(self.calendar[date]):
                ...
                # Compile 'Date', 'Name', 'Description' into list of dicsts (events)
                events.append({
                ...
                #     fieldnames = ['Date', 'Name', 'Description']
...
class LiturgicalCalendar:
    ...
    def to_csv(self, html_formatting=False):
        """Write out the liturgical calendar to CSV format.
        ...
        with open('test.csv', 'w', newline='') as csvfile:
            ...
            for year in self.liturgical_years:
                ics_year = self.liturgical_years[year].to_list(html_formatting)

                for event in ics_year:
                    writer.writerow({
                        'Date': event['Date'],
                        'Name': event['Name'],
